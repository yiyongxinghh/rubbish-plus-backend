...
export class GarbageService {
  ...
  updateBatch(updateGarbageDtos: UpdateGarbageDto[]) {
    return updateGarbageDtos.map(data => {
      this.garbage.createQueryBuilder().update(Garbage).set(data).where("garbage_id = :id", { id: data.garbageId }).execute();
    })
  ...
  getTotal() {
    return this.garbage.createQueryBuilder().getCount() // 获取总数量
  }
